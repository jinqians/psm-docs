...
在 [PSM 面板](/features/panel)里，新建节点时勾选**挂到 443 端口复用**即可，面板会把它交给服务器上的 psm-agent 去做，和命令行是同一条路径。只有能挂的协议才会出现这个勾选框；勾上以后「连接端口」固定为 443，不用再填。

::: warning 建好之后不能改
一个节点是直连自己的端口、还是挂在 443 复用上，决定了它对外的地址，所以这不是「修改」能完成的事——PSM 和面板都会拒绝。要换过来，把节点删掉再按新的方式建一个。
:::
